...
VulkanRenderer::QueueFamilyIndices VulkanRenderer::findQueueFamilies(VkPhysicalDevice _device) {
...
VulkanRenderer::SwapChainSupportDetails VulkanRenderer::querySwapChainSupport(VkPhysicalDevice _device) {
...
std::vector<char> VulkanRenderer::readFile(const std::string &_filename) {
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer _command_buffer, uint32_t _image_index) {
    ...
    renderPassInfo.framebuffer = swapChainFramebuffers[_image_index];
...
bool VulkanRenderer::isDeviceSuitable(VkPhysicalDevice _device) {
    ...
    return // List of conditions for a viable device.
            indices.isComplete() &&
            extensionsSupported &&
            swapChainAdequate;
}
